...
class MainApplication(ttk.Frame):
	...
	def __init__(self, parent, *args, **kwargs):
		...
		self.footer_text = Label(self, height=5, wraplength=600, textvariable=self.message)
		self.footer_text.grid(row=4, column=0, columnspan=2)

		copy_files_db.create_db(self)
	...
	def moveFiles(self, x, f):
		...
		print(self.check_the_time)
		if self.check_the_time < timeToCheck:
...
if __name__ == "__main__":
	...
	root.mainloop()
		...
		# file.add_command(label = 'Open', command = lambda: self.create_window(self.counter))
	...
	# def create_window(self, self.counter):
	# 	self.counter = self.counter + 1
	# 	t = ttk.Toplevel(self)
	# 	t.wm_title('Window number {}'.format(self.counter))
	# 	l = ttk.Label(t, text='This is window #{}'.format(self.counter))
