...
def first_run(self):
	...
	with conn:
		...
		if count < 1:
					# Is this the correct way to pass in argument?
			cur.execute("""INSERT INTO tbl_phonebook (col_fname, col_lname, col_fullname,col_phone,col_email) VALUES(?,?,?,?,?)""", ('John','Doe','John Doe', '111-11-1111','jdoe@email.com'))
			conn.commit()
	conn.close()
	...
	if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
		...
		with conn:
			...
			if chkName == 0:
				...
				messagebox.showerror("Nmae Error", "'{}' already exists in the database!".format(var_fullname))
				onClear(self)
		conn.commit()
		conn.close()
	else:
...
def onUpdate(self):
	...
	if (len(var_phone) > 0) and (len(var_email) > 0):
		...
		messagebox.showerror('Missing info', 'Please select a name')
	onClear(self)
